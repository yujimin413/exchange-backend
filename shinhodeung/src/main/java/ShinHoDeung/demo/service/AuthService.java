...
import ShinHoDeung.demo.common.GlobalVariable;
...
import ShinHoDeung.demo.provider.APIProvider;
...
public class AuthService {
    ...
    public UsaintAuthReturnDto uSaintAuth(UsaintAuthParamDto usaintAuthLoginDto)throws APIRequestFailedException, AuthFailedException, HTMLParseFailedException{
        ...
        for(Element uSaintPortalInfoBoxLi : uSaintPortalInfoBoxLis){
            ...
            else if(dt.text().equals("과정/학기")){
                usaintAuthReturnDto.setStatus(strong.text());
            }
            else if(dt.text().equals("학년/학기")){
                usaintAuthReturnDto.setSemester(strong.text());
            }
